cfrom gpytorch.kernels.kernel import Kernel
...
class LadderKernel(Kernel):
        def __init__(self, base_latent_kernel, strucutred_kernel, latent_train, train_smiles, **kwargs):
            ...
            self.structured_kernel = strucutred_kernel
        ...
        def forward(self, z1, z2, **params):
            ...
            K_z1_training = self.structured_kernel(smiles_z1, self.train_smiles) 
            K_z2_training = self.structured_kernel(smiles_z2, self.train_smiles) 
            K_train_smiles = self.structured_kernel(self.train_smiles, self.train_smiles)
